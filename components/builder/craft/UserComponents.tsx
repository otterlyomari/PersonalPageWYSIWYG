// components/builder/craft/UserComponents.tsx
"use client";

import { useNode } from "@craftjs/core";
import React from "react";

/* --- 1. CRAFT TEXT / HEADING COMPONENT --- */
...
/* --- 2b. CRAFT PROFILE INFO COMPONENT --- */
...
export const CraftProfileInfo = (_props: CraftProfileInfoProps) => {
  ...
  return (
    <div ref={(ref) => { if (ref) connect(drag(ref)); }} className="text-center space-y-1 p-2 cursor-pointer">
      ...
      <p className="text-sm opacity-80">{bio && bio.trim() !== "" ? bio : "No bio provided."}</p>
    </div>
  );
};

const ProfileInfoSettings = () => {
  const { actions: { setProp }, username, bio } = useNode((node) => ({
    username: node.data.props.username,
    bio: node.data.props.bio,
  }));
...
CraftProfileInfo.craft = {
  props: { username: "username", bio: "No bio provided." },
  related: {
...
/* --- 3. LINK CARD COMPONENT --- */
interface CraftLinkCardProps {
  title: string;
  url: string;
}

export const CraftLinkCard = ({ title, url }: CraftLinkCardProps) => {
  const { connectors: { connect, drag } } = useNode();
  return (
    <div ref={(ref) => { if (ref) connect(drag(ref)); }} className="w-full py-3 px-4 rounded-xl bg-white/10 border border-white/5 shadow-sm text-center font-medium text-sm cursor-pointer hover:bg-white/20 transition">
      <span>{title}</span>
    </div>
  );
};

const LinkCardSettings = () => {
  const { actions: { setProp }, title, url } = useNode((node) => ({
  ...
  return (
    <div className="space-y-3">
      <div className="space-y-1">
        <label className="text-[10px] font-mono uppercase opacity-50 block">Link Title</label>
        <input
          type="text"
          value={title || ""}
...
CraftLinkCard.craft = {
  props: { title: "My Portfolio", url: "https://example.com" },
  related: {
    settings: LinkCardSettings,
    toolbar: LinkCardSettings,
  },
};
